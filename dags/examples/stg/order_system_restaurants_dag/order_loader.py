...
class OrdersLoader:
    ...
    def __init__(
        self,
        collection_loader: OrdersReader,
        pg_dest: PgConnect,
        orders_pg_saver: OrdersPgSaver,
        logger: Logger,
    ) -> None:
        self.collection_loader = collection_loader
        self.orders_pg_saver = orders_pg_saver
    ...
    def run_copy(self) -> int:
        with self.pg_dest.connection() as conn:
            ...
            load_queue = self.collection_loader.get_orders(
                last_loaded_ts, self._SESSION_LIMIT
            )
            self.log.info(
                f"Found {len(load_queue)} documents to sync from orders collection."
            )
            if not load_queue:
            ...
            i = 0
            for d in load_queue:
                self.orders_pg_saver.save_object(conn, str(d["_id"]), d["update_ts"], d)

                i += 1
                if i % self._LOG_THRESHOLD == 0:
                    self.log.info(
                        f"processed {i} documents of {len(load_queue)} while syncing orders."
                    )
